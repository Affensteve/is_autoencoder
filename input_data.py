...
# ==============================================================================
"""Functions for downloading and reading MNIST MNIST_data (deprecated).
...
@deprecated(None, 'Please use tf.MNIST_data to implement this functionality.')
def _extract_images(f):
  ...
  Returns:
    MNIST_data: A 4D uint8 numpy array [index, y, x, depth].
...
class _DataSet(object):
  ...
  def __init__(self,
    ...
    Args:
      images: The images
      labels: The labels
      fake_data: Ignore inages and labels, use fake MNIST_data.
  ...
  def next_batch(self, batch_size, fake_data=False, shuffle=True):
    """Return the next `batch_size` examples from this MNIST_data set."""
    if fake_data:
      fake_image = [1] * 784
      if self.one_hot:
        fake_label = [1] + [0] * 9
...
def _maybe_download(filename, work_directory, source_url):
  """Download the MNIST_data from source url, unless it's already here.
